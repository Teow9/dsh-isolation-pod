 ...
 * confined to a user-specified sandbox root. Ported from the dynamic-plugin
 * version; the only Channel change is the RPC transport (an HTTP route instead
 * of `harness.handle`) and durable config/task state.
 *
...
const KNOWN_TOOLS = [
  ...
  'present', 'pwsh', 'bash', 'ralph', 'read', 'read_image', 'send_message', 'skill', 'subagent',
...
const BUILD = 5            // bumped on every host-half change; reported by /status
const MAX_TASKS = 200
...
export function apply(ctx) {
      ...
      if (Array.isArray(parsed.tasks)) {
        for (const raw of parsed.tasks.slice(-MAX_TASKS)) {
          ...
          const task = {
            ...
            sandboxCheck: { mode: str(raw.sandboxMode), fsDenied: raw.fsDenied === true, note: '' },
    ...
    try {
      ...
      if (str(event.type) === 'request/header') {
        try {
          ...
          for (const tool of list) observedTools[str(tool.name)] = true
        } catch { /* ignore */ }
      }
      const entry = summarize(event)
  ...
  const guardFor = (allow) => (execution) => {
    try {
  ...
  const sandboxSelfTest = async (session) => {
    const result = { mode: '', root: '', fsDenied: false, note: '' }
    try {
      ...
      if (result.mode !== want) { result.note = '策略模式不符（期望 ' + want + '）'; return result }
      const parent = config.sandboxRoot.replace(/[\\/][^\\/]*$/, '')
      if (parent !== '' && parent !== config.sandboxRoot) {
        const probePath = parent + '\\__ipp_probe_' + nowId() + '.txt'
        try {
          ...
          if (code === 'FS_SANDBOX_DENIED' || message.indexOf('file access denied') >= 0) result.fsDenied = true
          else result.note = '越界探测结果不明确：' + message.slice(0, 160)
        }
      }
    } catch (error) { result.note = '沙箱自检失败：' + errText(error) }
    return result
  }
  ...
  const runTask = async (task) => {
    ...
    try {
      ...
      try {
        ...
        if (selection) agentOptions = { provider: str(selection.provider), model: str(selection.model) }
      ...
      handle = await ctx.agents.create({
        ...
        setup: async (agentCtx, agent) => {
          ...
          agentCtx.tools.guard(guardFor(allow))
          agentCtx.systemPrompt.section({
            name: 'isolation-pod/rules',
            order: 900,
            text: 'ISOLATION POD（隔离舱）：你是运行在主会话之外的隔离执行环境。唯一可写位置是 ' + config.sandboxRoot + '，其余路径一律只读，越界写入会被系统强制拒绝。只能使用这些工具：' + allow.join(', ') + '，其它工具会被拒绝。所有输出写入 ' + task.taskDir + '。'
          })
        }
      })
      ...
      // guard above remains the authoritative allowlist.
      try {
        const restrictTo = ['read', 'write', 'edit', 'glob', 'grep', 'pwsh', 'bash'].filter((tool) => allow.includes(tool))
    ...
    await runTurn(task, handle, prompt, 'instructions')
  }
   ...
   */
  const runTurn = async (task, handle, text, form) => {
    ...
    try {
      ...
      handle.agent.followup({
        id: 'ipp-' + task.id + '-t' + String(Number(task.turns) || 1),
        role: 'user',
        content: [{ type: 'text', text }],
        source: { kind: 'plugin', plugin: 'isolation-pod', form: form === 'relay' ? 'relay' : 'instructions' }
      })
      await handle.agent.whenIdle()
  ...
  const taskSummary = (task) => ({
    ...
    contextNote: task.contextNote || '', allow: task.allow || [], logCount: task.log.length,
    turns: Number(task.turns) || 1,
  ...
  const methods = {
    ...
    async setConfig(args) {
      ...
      if (patch.sandboxRoot !== undefined) {
        const value = str(patch.sandboxRoot).trim()
    ...
    async followUp(args) {
      ...
      task.startedAt = Date.now()
      scheduleSave()
      runTurn(task, handle, text, 'relay').catch((error) => {
        task.status = 'failed'
        task.error = errText(error)
        task.endedAt = Date.now()
        scheduleSave()
    ...
    async returnToMain(args) {
      ...
      agent.followup({
        ...
        source: { kind: 'plugin', plugin: 'isolation-pod', form: 'relay' }
      })
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (pathname === '/isolation-pod/status') {
        ...
        send(res, 200, {
          ok: true, build: BUILD, tokenPrefix: token.slice(0, 8), taskCount: tasks.size, storePath,
  ...
  ctx.effect(() => ctx.webServer.tapIndex((html) => {
    const tag = '<script>window.__DSH_IPP__=' + JSON.stringify({ token }) + ';</script>'
  ...
  loadState()
  console.log('[isolation-pod] host half ready; state at ' + storePath)
}
