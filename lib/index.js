...
const BUILD = 7            // bumped on every host-half change; reported by /status
...
export function apply(ctx) {
  ...
  /**
   * Names of the tools one agent context can actually see, in registry order.
   *
   * A pod's tools come from the preset mounted onto it — on the Web/Desktop
   * surface the host plane registers none of them — so this is the only honest
   * source for "what may this pod call".
   */
  const surfaceTools = (agentCtx) => {
    try {
      const rows = agentCtx.tools.schemas()
      const out = []
  ...
  /**
   * Narrow a configured allowlist to what the pod can actually call.
   *
   * An empty surface means "no preset mounted, or the probe failed": the
   * configured list is then kept verbatim so the panel still shows the request
   * and the guard stays the single authority. A non-empty surface wins — an
   * empty intersection is the truth (the preset offers none of the ticked
   * tools) and the panel shows `availableTools` beside it.
   */
  const intersectAllow = (allow, surface) => {
    if (!Array.isArray(surface) || surface.length === 0) return allow
    return allow.filter((tool) => surface.includes(tool))
  }
  ...
  const buildPrompt = (task, contextText) => {
    ...
    lines.push('可用工具（其它工具已被拒绝）：' + (task.allow.length > 0
      ? task.allow.join(', ')
      : '（无：当前 preset 未提供白名单里的任何工具，请在面板调整白名单或 preset）'))
    ...
    try {
      ...
      handle = await ctx.agents.create({
        ...
        setup: async (agentCtx, agent) => {
          ...
          // this view is the pod's real surface. Narrow the allowlist to it
          // before the guard starts consulting `task.allow`.
          task.availableTools = surfaceTools(agentCtx)
          task.allow = intersectAllow(requested, task.availableTools)
