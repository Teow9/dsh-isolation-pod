 ...
 * window.__DSH_IPP__, injected by the host half as a structured index-injection
 * row (so it reaches both the served Web page and the Desktop renderer), with
 * `/isolation-pod/bootstrap` and a single guarded reload as recovery paths.
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			'.ipp-preview{margin-top:10px;border-top:.5px dotted var(--dsw-alias-border-l1);padding-top:8px}',
			'.ipp-pre{font-family:var(--ds-font-family-code,monospace);font-size:12px;line-height:1.5;white-space:pre-wrap;word-break:break-word;max-height:320px;overflow:auto;background:var(--dsw-alias-bg-layer-2);border:.5px solid var(--dsw-alias-border-l1);border-radius:8px;padding:8px;margin:8px 0 0}'
		].join('')

		const TABS = [['tasks', '任务'], ['new', '新建'], ['config', '配置'], ['exports', '导出记录']]

		/*
		 * The bearer token reaches the page as a structured index-injection row
		 * (`webserver/index-inject` → `{ kind: 'global' }`), which the served Web
		 * form renders into the document head and the Desktop shell ships to its
		 * renderer over IPC. That covers a page as it LOADS — and a page loaded
		 * before this plugin existed (or one whose Host restarted) has no token.
		 *
		 * Two recovery paths follow, in order:
		 *   1. `/isolation-pod/bootstrap` — the Host hands the token back to a
		 *      session the harness itself authenticated. The Desktop forwards its
		 *      host cookie on every `dsh-app://app/*` request and a Web browser
		 *      carries the same cookie, so this works on both surfaces.
		 *   2. A single guarded reload, which re-runs the boot injection. This is
		 *      the last resort because release Desktop builds expose no reload
		 *      command at all — the page must do it for itself.
		 */
		let tokenCache = (window.__DSH_IPP__ && window.__DSH_IPP__.token) || ''

		const TOKEN_MISSING = '隔离舱令牌不可用：请重试，或退出并重新打开桌面应用（Web 版请重新打开 dsh web 打印的 URL）。'
		const TOKEN_STALE = '隔离舱令牌已失效：请退出并重新打开桌面应用（Web 版请重新打开 dsh web 打印的 URL）。'

		const isDesktopShell = () => {
			try { return !!(globalThis.__DSH_TRANSPORT__ && globalThis.__DSH_TRANSPORT__.ownsHost === true) } catch (error) { return false }
		}

		const bootstrapToken = () => fetch('/isolation-pod/bootstrap', { credentials: 'same-origin', cache: 'no-store' })
		...
		/* Legacy/Web path: the served index carries the token when the Host renders
		 * it itself. Harmless in the Desktop shell, where it simply finds nothing. */
		const scrapeToken = () => fetch('/', { credentials: 'same-origin', cache: 'no-store' })
		...
		/** One reload per few seconds, so a broken deployment cannot loop. */
		const reloadOnce = () => {
			try {
				const last = Number(window.sessionStorage.getItem('ipp-token-reload') || '0')
				if (Number.isFinite(last) && Date.now() - last < 5000) return false
				window.sessionStorage.setItem('ipp-token-reload', String(Date.now()))
			} catch (error) { return false }
			window.location.reload()
			return true
		}

		const refreshToken = () => bootstrapToken().then((found) => (found ? true : scrapeToken()))
		...
		const api = (method, args) => post(method, args)
			.then((response) => {
				...
				return refreshToken().then((found) => {
					if (found) {
						return post(method, args).then((retry) => (retry.status === 403
							? { ok: false, message: TOKEN_STALE }
							: retry.json()))
					}
					if (tokenCache === '' && reloadOnce()) return { ok: false, message: '正在重新载入页面以获取令牌…' }
		 ...
		 *
		 * A root-scoped panel gets no Session binding of its own — the `main` slot
		 * only binds the reserved `conversation` key — so the panel resolves it the
		 * way the shipped panels do: the one session retained by the main view.
		 * That reference is held while a panel is open (`replaceMain` keeps its
		 * reference; opening a panel only deselects the highlight), so it is valid
		 * here. Older shells exposed a plain `current` field instead, which 0.1.7
		 * no longer has — hence the two-step selector.
		 */
		const selectMainSession = (state) => {
			if (!state) return ''
			const byId = state.byId
			if (byId) {
				for (const id of Object.keys(byId)) {
					const row = byId[id]
					if (row && row.retainedBy && (row.retainedBy.mainView || 0) > 0) return String(row.id || id)
				}
			}
			return state.current ? String(state.current) : ''
		}

		function Panel(props, ctx) {
			const useSessions = props.useSessions
			const currentSessionId = useSessions ? useSessions(selectMainSession) : ''
			...
			const start = () => {
				...
				api('startTask', { mainSessionId: String(currentSessionId), description: desc, subdir: subdir }).then((result) => {
					if (result && result.ok) {
						...
						logCursor.current = 0; setDetailId(result.taskId); setFilePreview(null)
			...
			/*
			 * Read one sandbox file's text. The Host enforces the sandbox boundary and
			 * caps the read (≤300 KB on disk, maxChars clamped to ≤20000), so this is a
			 * convenience view, not an escape hatch.
			 */
			const openPreview = (relPath) => {
				if (!detail) return
				const taskId = detail.task.id
				setFilePreview({ relPath: relPath, loading: true, text: '', bytes: 0, truncated: false, error: '' })
				api('readSandboxFile', { taskId: taskId, relPath: relPath, maxChars: 20000 }).then((result) => {
					if (result && result.ok) {
						setFilePreview({
							relPath: relPath, loading: false, text: result.text || '',
							bytes: Number(result.bytes) || 0, truncated: !!result.truncated, error: ''
						})
					} else {
						setFilePreview({ relPath: relPath, loading: false, text: '', bytes: 0, truncated: false, error: (result && result.message) || '读取失败' })
					}
				})
			}
			...
			const detailView = (detail && detail.task) ? h('div', null,
				h('div', { className: 'ipp-card' },
					h('div', { className: 'ipp-row' },
						...
						detail.task.sandboxProbe === 'skipped-no-parent'
							? h('span', { className: 'ipp-chip warn' }, '未做越界写自检（沙箱根是盘符根）') : null,
						h('span', { style: { flex: 1 } }),
						h('button', { className: 'ipp-btn', onClick: () => { setDetailId(''); setDetail(null); setFiles(null); setPicked({}); setRetPreview(''); setFilePreview(null) } }, '返回列表')),
					h('div', { className: 'ipp-mut' }, '沙箱：' + detail.task.taskDir),
					detail.task.error ? h('div', { className: 'ipp-chip err' }, detail.task.error) : null,
					detail.task.setupNote ? h('div', { className: 'ipp-chip warn' }, detail.task.setupNote) : null,
					detail.task.contextNote ? h('div', { className: 'ipp-mut' }, '主会话上下文：' + detail.task.contextNote) : null,
					h('div', { className: 'ipp-row', style: { marginTop: 8 } },
						h('span', { className: 'ipp-lab' }, '可用工具'),
						h('span', { className: 'ipp-mut' }, (detail.task.allow || []).length ? (detail.task.allow || []).join(', ') : '（无：当前 preset 未提供白名单里的任何工具）')),
					(detail.task.availableTools && detail.task.availableTools.length)
						? h('div', { className: 'ipp-row' },
							h('span', { className: 'ipp-lab' }, '隔离环境实有'),
							h('span', { className: 'ipp-mut' }, detail.task.availableTools.join(', ')))
						: null),
				h('div', { className: 'ipp-card' },
					h('div', { className: 'ipp-row' },
						h('strong', null, '对话'),
				...
				h('div', { className: 'ipp-card' },
					...
					(files || []).length ? (files || []).map((file) => h('div', { key: file.relPath, className: 'ipp-row' },
						...
						file.type === 'file' ? h('button', {
							className: 'ipp-btn',
							onClick: () => (filePreview && filePreview.relPath === file.relPath ? setFilePreview(null) : openPreview(file.relPath))
						}, (filePreview && filePreview.relPath === file.relPath) ? '收起' : '预览') : null))
						: h('div', { className: 'ipp-mut' }, '（暂无文件，或尚未刷新）'),
					filePreview ? h('div', { className: 'ipp-preview' },
						h('div', { className: 'ipp-row' },
							h('strong', null, filePreview.relPath),
							h('span', { className: 'ipp-mut' }, filePreview.loading ? '读取中…'
								: (filePreview.error ? '' : (filePreview.bytes + ' B' + (filePreview.truncated ? ' · 已截断显示' : '')))),
							h('span', { style: { flex: 1 } }),
							h('button', { className: 'ipp-btn', onClick: () => setFilePreview(null) }, '关闭')),
						filePreview.error ? h('div', { className: 'ipp-chip err' }, filePreview.error) : null,
						(!filePreview.loading && !filePreview.error) ? h('pre', { className: 'ipp-pre' }, filePreview.text) : null) : null),
				h('div', { className: 'ipp-card' },
					h('strong', null, '返回主会话'),
			...
			const configView = (state && cfg) ? h('div', null,
				h('div', { className: 'ipp-card' },
					...
					h('div', { className: 'ipp-row' },
						...
						(state.presetChoices && state.presetChoices.length)
							? h('select', {
								className: 'ipp-in', value: cfg.presetId || '',
								onChange: (event) => setCfg(Object.assign({}, cfg, { presetId: event.target.value }))
							},
								h('option', { value: '' }, '（不挂载 preset）'),
								state.presetChoices.map((row) => h('option', { key: row.id, value: row.id },
									row.name ? (row.id + ' — ' + row.name) : row.id, row.broken ? '（不可用）' : '')))
			...
			return h('div', { className: 'ipp-root' },
				...
				h('div', { className: 'ipp-body' },
					error ? h('div', { className: 'ipp-warn' },
						h('div', null, '隔离舱接口暂时不可用：' + error),
						h('div', { className: 'ipp-row', style: { marginTop: 8 } },
							h('button', { className: 'ipp-btn', onClick: () => window.location.reload() }, '重新载入页面'),
							h('span', { className: 'ipp-mut' }, isDesktopShell()
								? '桌面版：重新载入会重新获取令牌；仍失败请退出并重新打开应用。'
								: 'Web 版：重新载入会重新获取令牌；仍失败请重新打开 dsh web 打印的 URL。'))) : null,
					body))
		}
